    ...
    implements IFieldsProvider, IWizardDataEditorProvider
{
     ...
     */
    @Override
    public void setDataAdapter(final DataAdapter dataAdapter)
    {
    ...
    /**
     * Return the icon for the data adapter or null if the requested icon is not
     * available
     */
    @Override
    ...
    /**
     * Return the list of fields provided by the data adapter with the current
     * configuration. See the comments on the superclass for more detailed
     * informations
     *
     * @param con the data adapter service
     * @param the current jasper reports configuration
     * @param the dataset generated by the data adapter service, can be null
     *            depending on how is implemented the fields provider
     *
     * @return a not null list of JRDesignFields
     */
    @Override
    public List<JRDesignField> getFields(final DataAdapterService con,
                                         final JasperReportsConfiguration jConfig,
                                         final JRDataset reportDataset)
        throws JRException, UnsupportedOperationException
    {
        getFieldProvider();
        return this.fieldsProvider.getFields(con, jConfig, reportDataset);
    }
    ...
    {
        if (this.fieldsProvider == null)
            this.fieldsProvider = new EFapsAdapterFieldsProvider();
    }
     ...
     *
     * @return true if the fields can be retrieved automatically with the
     *         getFields functions, false otherwise
     */
    @Override
    public boolean supportsGetFieldsOperation(final JasperReportsConfiguration jConfig)
    {
        getFieldProvider();
        return this.fieldsProvider.supportsGetFieldsOperation(jConfig);
    }

    /**
     * Return the wizard editor used to configure the data adapter during the
     * new report wizard. Can return null if no custom wizard is provided
     *
     * @return the controls to edit the adapter during the new report wizard or
     *         null
     */
    @Override
    public AWizardDataEditorComposite createDataEditorComposite(final Composite parent,
